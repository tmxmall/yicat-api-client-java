package vip.yicat.client.core.http;
...
import vip.yicat.client.core.model.EnumConverter;
...
public interface HttpClient {
    ...
    <T, V> T get(String url, V data, HttpRequestConfig config, Class<T> clazz) throws HttpException, HttpBadRequestException;

    <T> T delete(String url, HttpRequestConfig config, Class<T> clazz) throws HttpException, HttpBadRequestException;

    <T, V> T delete(String url, V data, HttpRequestConfig config, Class<T> clazz) throws HttpException, HttpBadRequestException;
     ...
     * @param clazz  return class type (internally http client will work with java classes without generic type, like {@link })
     * @param <T>    return type
    ...
    InputStream executeDownload(String url, HttpRequestConfig config);
